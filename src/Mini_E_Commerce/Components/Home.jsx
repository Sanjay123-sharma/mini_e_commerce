...
export const ShowList=({ProductList,error,loading})=>{
  ...
  return(
    <>
      <div className="p-6">
  <h1 className="text-3xl font-bold mb-6 bg-color-black">Home Page</h1>
  <NavLink to={'/addToCart'} style={{color:'blue'}}>Selected List</NavLink>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {
            ProductList.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl shadow-md p-4 hover:shadow-lg transition">
                ...
                <div className="text-blue-500"><NavLink to={`/product/${item.id}`}>view More</NavLink></div>
                <div><button className='bg-green-400 hover:bg-blue-700 text-white font-bold py-2 px-2 rounded ' onClick={()=>hanldeAdd(item.id)}>ADD</button></div>
              </div>
            ))
          }
        </div>
